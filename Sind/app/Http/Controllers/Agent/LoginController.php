...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Hash::check(strtolower($request->input('captcha')), $request->session()->get('captcha')['key'])) {
            ...
            if ($this->attemptLogin($request)) {
                if ($this->guard()->user()->role_id != 2) {
                    ...
                    $request->attributes->set('error', '您不是代理,请确认身份!');
            ...
            $request->attributes->set('error', '验证码错误');
        }
    ...
    public function secure(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (!Hash::check($request->input('old_password'), auth('agent')->user()->password)) {
                return ['success' => false,'msg'=>'旧密码错误'];
            }

            if (Agent::query()->where('id', auth('agent')->user()->id)->update(['password'=>bcrypt($request->input('password'))])) {
                return ['success' => true,'msg'=>'修改成功'];
            }
            return ['success' => false,'msg'=>'修改失败'];
        }
        return view('agent.secure');
    }
}
