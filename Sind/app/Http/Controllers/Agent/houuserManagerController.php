...
class houuserManagerController  extends Controller
{
...
//               ->whereDate("create_time","<=",$end_time);
    ...
    public function teamStat_hou(Request $request) {
        $agent_id=$request->agent_id;
        $agent_info=User::query()->where('user_id',$agent_id)->first();
        $dl_level =$agent_info['dl_level'];
        if($dl_level==2){
...
//            $agent_id = auth('agent')->user()->user_id;
            ...
            return view('agent.teamMsg.teamStat2',[
                'data'      => $data,
                "all_data"  => $all_data,
                "time"      => $time,
                "val"       => $val,
...
//            $agent_id = auth('agent')->user()->user_id;
            ...
            foreach ($teamList as $item){
                ...
                $rolid= BuyUser::query()->where('user_id',$item)->value('role_id');
               $list_all['username'] = $itemUsername;
               $list_all['user_id'] = $item;
               $list_all['role_id'] = $rolid;
