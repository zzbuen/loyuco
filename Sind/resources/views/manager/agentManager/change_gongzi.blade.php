...
<div class="layui-everyday-list">
    <form class="layui-form layui-form-pane" action="" style="margin-left: 20%;margin-top: 10%">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="number" id="riliang" name="source_name" value="{{$old_gongzi["amount_day"]}}" lay-verify="required"  autocomplete="off" class="layui-input">
                <input type="hidden" id="riliang_hidden" name="source_name" value="{{$old_gongzi["amount_day"]}}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">活跃人数</label>
            <div class="layui-input-inline">
                <input type="number" id="renshu" name="source_name"  value="{{$old_gongzi["amount_num"]}}" lay-verify="required"  autocomplete="off" class="layui-input">
                <input type="hidden" id="renshu_hidden" name="source_name" value="{{$old_gongzi["amount_num"]}}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">活跃金额</label>
            <div class="layui-input-inline">
                <input type="number" id="jiner" name="source_name"   value="{{$old_gongzi["amount_money"]}}" lay-verify="required"  autocomplete="off" class="layui-input">
                <input type="hidden" id="jiner_hidden" name="source_name" value="{{$old_gongzi["amount_money"]}}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">日工资比例</label>
            <div class="layui-input-inline">
                <input type="number" id="bili" name="source_name" value="{{$old_gongzi["wage_ratio"]}}" lay-verify="required"  autocomplete="off" class="layui-input">
            </div>
        </div>
        <input type="hidden" id="old_gongzi" value="{{$old_gongzi["wage_ratio"]}}">
        <input id="wage_limit" type="hidden" value="{{$gongzi}}">
    ...
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        $(document).on("click","#qingding",function () {
            ...
            if(riliang - riliang_hidden>0){
                layer.msg("日量不可高于之前的日量数据"+riliang_hidden);
                return false;
            }
            if(renshu<0 || renshu==""){
                layer.msg("人数不可小于0哦");
            ...
            if(bili-0>wage_limit){
                layer.msg("日工资比例不能超过"+wage_limit+"%");
                return false;
            }
            if(bili-0<0){
                layer.msg("日工资比例不能小于0哦亲");
                return false;
            }
            if(bili-0<old_gongzi){
                layer.msg("日工资比例不能小于之前的比例"+old_gongzi+"%哦亲");
                return false;
            }
            $.ajax({
                type:"post",
                url:"{{url('manager/ajax_change_gongzi')}}",
                data:{
                    "user_id":user_id,
                    "riliang":riliang,
                    "renshu":renshu,
