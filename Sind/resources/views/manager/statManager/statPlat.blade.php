            ...
            <div class="layui-table">
                        ...
                        <div class="info-box">
                            ...
                            <div class="info-box-content">
                                ...
                                <span class="info-box-number">
                                    <span style="color: red;">{{number_format($data["winning"]-$data["betting"],2)}}</span>
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="layui-form" >
                    <table class="layui-table">
                        <thead>
                            <tr>
                                <th class=" sorting" sort="daymoney">统计日期</th>
                                <th class=" sorting" sort="server_charge">充值总额</th>
                                <th class=" sorting" sort="share_profit">提款总额</th>
                                <th class=" sorting" sort="share_profit">投注总额</th>
                                <th class=" sorting" sort="share_profit">彩金总额</th>
                                <th class=" sorting" sort="pure_profit">派奖总额</th>
                                <th class=" sorting" sort="pure_profit">彩票盈亏</th>
                                <th class=" sorting" sort="pure_profit">实际盈亏</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($all_data as $key=>$value)
                                <tr>
                                    <td class=" sorting" sort="daymoney">{{$value["create_time"]}}</td>
                                    <td class=" sorting" sort="server_charge">{{$value["recharge"]}}</td>
                                    <td class=" sorting" sort="share_profit">{{$value["withdrawals"]}}</td>
                                    <td class=" sorting" sort="share_profit">{{$value["betting"]}}</td>
                                    <td class=" sorting" sort="share_profit">{{$value["bonus"]}}</td>
                                    <td class=" sorting" sort="pure_profit">{{$value["winning"]}}</td>
                                    <td class=" sorting" sort="pure_profit">
                                        @if($value["total"]-$value["bonus"]<0)
                                            <span style="color: red">             {{$value["total"]-$value["bonus"]}}</span>
                                            @else
                                            {{$value["total"]-$value["bonus"]}}
                                        @endif
                                    </td>
...
<script>
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        $(document).on("click","#select",function () {
            ...
            location.href = "{{url('manager/stat.plat.index') }}?val="+val+'&time=x';
        });
