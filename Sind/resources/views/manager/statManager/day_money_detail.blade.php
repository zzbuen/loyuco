...
<div class="layui-fluid main">
    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
        <legend>每日盈亏详情</legend>
    </fieldset>
    <form method="post" action="{{ url("manager/day_money_detail") }}?time={{Request::input('time')}}">
        {{ csrf_field() }}
        ...
        <table class="layui-table">
            <thead>
                <tr>
                    ...
                    <th>用户ID</th>
                    <th>用户名</th>
                    <th class="{{ Request::get('column')=='bet_money'?(Request::get('sort')=='asc'?'sorting_asc':'sorting_desc'):'sorting_both' }} sorting" sort="bet_money">下注总额</th>
                    ...
                    <th class="{{ Request::get('column')=='daymoney'?(Request::get('sort')=='asc'?'sorting_asc':'sorting_desc'):'sorting_both' }} sorting" sort="daymoney">盈亏</th>
                </tr>
            </thead>
        <tbody>
            ...
            <tr class="day_income_user">
                ...
                @else
                    <td style="color: red">{{substr($item['result'],1)}}</td>
                @endif
            </tr>
            @endforeach
        </tbody>
...
<script>
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        $(".day_income_user").click(function () {
            ...
            var time = '{{$time}}'
            location.href = '{{url('manager/day_income_user')}}?time='+time+'&user_id='+user_id
        })
