...
<div class="layui-fluid main">
    <div class="layui-tab">
        <ul class="layui-tab-title">
            <li id="order">日工资</li>
            <li id="order_back" class="layui-this">分红</li>
        </ul>
        ...
        <div class="layui-form">
            ...
            <tbody>
                @foreach($data as $key=>$value)
                    <tr>
                        <th>{{$value["userinfo"]["username"]}}</th>
                        <th>
                            @if($value["agent_id"] == 0)
                                <span style="color: red">系统</span>
                            @else
                                {{$value["shangji"]["username"]}}
                            @endif
                        </th>
                        <th>
                        ...
                        <th>{{$value["total"]}}</th>
                        <th>{{$value["amount_num"]}}</th>
                        <th>{{$value["amount_money"]}}</th>
                        <th>{{$value["bonus_ratio"]}}%</th>
                        <th>{{$value["today_touzhu"]}}</th>
                        <th>
                            @if($value["status"] == 1)
                                已签订
                            @endif
...
<script>
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        $(document).on("click",".kj_time",function () {
            ...
            location.href = "{{url('manager/user_qiyue_fenhon') }}?time="+$data;
        });
        ...
        $(document).on("click","#select",function () {
            ...
            location.href = "{{url('manager/user_qiyue_fenhon') }}?val="+val+"&user_id="+user_id;
        });
        ...
        $(document).on("click","#order",function () {
            location.href = "{{url('manager/user_qiyue') }}";
        });
        ...
        $(document).on("click",".look_detail",function () {
            ...
            layer.open({
                ...
                area: ['50%', '80%'],
                maxmin: true,
                content: '{{ route("manager.fenhon.detail") }}?id='+id,
            });
        });
