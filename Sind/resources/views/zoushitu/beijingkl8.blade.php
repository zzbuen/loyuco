...
<div id="top">
    ...
    <div id="category">
        <ul>
            <li class="category color_greend" >常规形态</li>
        </ul>
    </div>
<div id="main">
    {{--遗漏辅助线近期等选择start--}}
            ...
            @if($data)
                ...
                <thead>
                    <tr style="height: 2.5rem">
                        ...
                        <th class="all_td border_bottm color_one" colspan="20">开奖结果</th>
                        <th class="all_td color_one max_right" rowspan="2">和值</th>
                        <th class="all_td color_one max_right" rowspan="2">跨度</th>
                        <th class="all_td border_bottm color_one" colspan="10">和值尾数</th>
                        ...
                        <th class="all_td color_one" rowspan="2" style="width: 3.75rem">升平降形态</th>
                    </tr>
                    {{--0-9显示start--}}
                    <tr style="height: 2.5rem">
                        @for($i=1;$i<=20;$i++)
                            @if($i==20)
                                <td class="border_bottm all_td" style="background: #3A3B50;color: white;">正{{$i}}</td>
                            @else
                                <td class="border_bottm" style="background: #3A3B50;color: white;">正{{$i}}</td>
                            @endif
                        @endfor
                        @for($i=0;$i<10;$i++)
                            @if($i==9)
                ...
                <tbody>
                    ...
                    @foreach($data as $key=>$value)
                        @php
                            $res_arr = explode(",",$value["result"]);
                            $sum     = array_sum($res_arr);
                            if($key!="0"){
                                $first_res = substr(array_sum(explode(",",$data[$key-1]["result"])),-1,1);
                            }else{
                                $first_res = 0;
                            }
                        ...
                        <tr class="show_border <?php if(($key+1)%5==0) echo 'border_bottm_5' ?>">
                            ...
                            @for($i=0;$i<20;$i++)
                                @php
                                    $res_one = substr($value["result"],3*$i,2);
                                @endphp
                                @if($i==19)
                                    <td class="all_td" style="padding: 0 0.6rem;color: indianred">{{$res_one}}</td>
                                    @else
                                    <td style="padding: 0 0.6rem;color: indianred">{{$res_one}}</td>
                                @endif
                            @endfor

                            <td class="all_td">{{$sum}}</td>
                            <td class="all_td">{{max($res_arr)-min($res_arr)}}</td>

                            {{--和值尾数start--}}
                            @for($i=0;$i<10;$i++)
                                @if(substr(array_sum($res_arr),-1,1) == $i)
                                    @php
                                        /*遗漏值清0*/
                                        $fenbu_arr[$i] = 0;
                                        /*平均总次数+1*/
                                    ...
                                    @endphp
                                    <td <?php if($i==9) echo 'class="all_td "';?>><span class="layui-badge xian">{{$i}}</span></td>
                                @else
                                    @php
                                        /*遗漏值++*/
                                        $fenbu_arr[$i]++;
                                    ...
                                    @endphp
                                    <td class="yiloutiao{{$i}}{{$fenbu_total_arr[$i][7]}} <?php if($i==9) echo 'all_td';?>" ><span class="yilou  ">{{$fenbu_arr[$i]}}</span></td>
                                @endif
                            @endfor
                            {{--和值尾数end--}}
                            ...
                            <td class="all_td <?php if($key+1 == $count_data){echo 'daxiao_noborder';}else{echo 'daxiao';}?>">
                                @if(substr(array_sum($res_arr),-1,1)>5)
                                    大
                                @else
                                    小
                                @endif
                            ...
                            <td class="all_td   <?php if($key+1 == $count_data){echo 'danshuang_noborder';}else{echo 'danshuang'; }?>">
                                @if(substr(array_sum($res_arr),-1,1)%2==0)
                                    双
                                @else
                                    单
                                @endif
                            ...
                            <td class="all_td <?php if($key+1 == $count_data){echo 'daxiao_noborder';}else{echo 'daxiao';}?>">
                                @php $zhishu = 0; @endphp
                                @for($i=2;$i<substr(array_sum($res_arr),-1,1);$i++)
                                    @if(substr(array_sum($res_arr),-1,1)%$i==0)
                                        @php $zhishu++ @endphp
                                    @endif
                                @endfor
                                @if($zhishu)
                            ...
                            {{--012形态--}}
                            <td class="all_td <?php if($key+1 == $count_data){echo 'danshuang_noborder';}else{echo 'danshuang';}?>">
                                {{substr(array_sum($res_arr),-1,1)%3}}
                            </td>

                            <td class="all_td <?php if($key+1 == $count_data){echo 'hezhi_noborder';}else{echo 'hezhi';}?>"style="width: 3.75rem">
                                @if($first_res<substr(array_sum($res_arr),-1,1))
                                    升
                                    @elseif($first_res==substr(array_sum($res_arr),-1,1))
                                    和
                                    @else
                                    降
                                @endif
                ...
                <tbody>
                    ...
                    <tr class="height40">
                        ...
                        <td rowspan="1" class="all_td border_bottm color_one" colspan="20"></td>
                        <td rowspan="1" class="all_td border_bottm color_one"></td>
                        <td rowspan="1" class="all_td border_bottm color_one"></td>
                        {{--号码分布平均总次数start--}}
                        @for($i=0;$i<10;$i++)
                    ...
                    <tr class="height40">
                        ...
                        <td class="all_td border_bottm color_one" colspan="20" ></td>
                        <td rowspan="1" class="all_td border_bottm color_one"></td>
                        <td rowspan="1" class="all_td border_bottm color_one"></td>
                        {{--平均遗漏值start--}}
                        @for($i=0;$i<10;$i++)
                ...
                <tbody>
                    ...
                    <tr style="height: 2.25rem">
                        <th class="all_td border_bottm color_one" colspan="20" rowspan="1" >开奖结果</th>
                        <th class="all_td border_bottm color_one" colspan="10" >和值尾数</th>
                        @if($xing==5 || $xing==6)
    ...
    <input id="arr" type="hidden" value="{{json_encode($fenbu_total_arr)}}">
</div>
...
<script type="text/javascript">
    ...
    function show_yiloutiao(arr,bg,color) {
        for(var i=0;i<arr.length;i++){
            var class_name = "yiloutiao"+i+arr[i][7];
    ...
    $(function () {
        ...
        $(".link_number").click(function () {
            ...
            location.href = "/beijingkl8?number="+$data;
        });
        ...
        $(".chaxun").click(function () {
            var $data = $("#test6").val();
            location.href = "/beijingkl8?time="+$data;
        });
