...
<div class="layui-fluid main">
    ...
    <form method="post" action="{{url('agent/agent_children')}}?leader_id={{Request::input('leader_id')}}">
        {{ csrf_field() }}
        ...
        <table class="layui-table">
            ...
            @foreach($user_list as $item)
                <tr>
                    ...
                    <td>否</td>
                    @endif
                    @if($item['remaining_money']||$item['remaining_money']==0)
                    <td>{{$item['remaining_money']}}</td>
                    @else
                    <td></td>
                    @endif
                    @if($item['unliquidated_money']||$item['unliquidated_money']==0)
                        <td>{{$item['unliquidated_money']}}</td>
                    @else
                        <td></td>
                    @endif
                    <td>
                        <input type="hidden" value="{{$item['id']}}"><a class="layui-btn getUser_detail">更多信息</a>
                        @if($item['parent_user_id'])
...
<script>
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        $(".sorting").click(function(){
            ...
            location.href = '{{ url("agent/agent_children") }}?user_type={{ Request::input("user_type") }}&leader_id={{ Request::input("leader_id") }}&column='+column+'&sort='+sort;
        })
        $("#back_agent").click(function () {
            location.href = '{{url('agent/agentCenter') }}';
        })
    });
</script>
