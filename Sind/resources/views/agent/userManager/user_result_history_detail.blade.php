...
<div class="layui-fluid main">
    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
        <legend>用户历史盈亏明细</legend>
    </fieldset>
    <form method="post" action="{{url('/agent/user_result_history_detail')}}?user_id={{Request::input('user_id')}}">
        {{ csrf_field() }}
...
<script>
    layui.use(['form', 'layedit', 'laydate'], function() {
        ...
        $(".sorting").click(function(){
            ...
            location.href = '{{ url("manager/user_result_history_detail") }}?bet_period={{ Request::input("bet_period") }}&date_end={{ Request::input("date_end") }}&date_begin={{ Request::input("date_begin") }}&user_id={{ Request::input("user_id") }}&order_sn={{ Request::input("order_sn") }}&column='+column+'&sort='+sort;
        })
        @if ($errors->has('error'))
